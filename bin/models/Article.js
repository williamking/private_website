...
(function(){
  var mongoose, ObjectId, CommentSchema, CommentModel, ArticleSchema, ArticleModel;
  ...
  ArticleModel.addComment = function(id, content, commentor, series, callback){
    ArticleModel.findOne({
      _id: id
    }, function(err, article){
      var newComent, index, reply, i$, to$, num;
      if (err) {
        callback(0, null);
      } else {
        if (article) {
          ...
          if (series !== '') {
            index = parseInt(series[0]);
            reply = article.comments[index];
            for (i$ = 1, to$ = series.length - 1; i$ <= to$; ++i$) {
              num = i$;
              index = parseInt(series[num]);
              reply = reply.replies[index];
            }
            reply.replies.push(newContent);
          } else {
